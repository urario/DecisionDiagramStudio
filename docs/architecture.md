...
| `BdtNodeCount` | `2^(VariableCount+1) - 1`（アプリ層算出） | BDD のみ |
| `ReducedCount` | `BdtNodeCount - ReachableNodeCount`（アプリ層算出） | BDD のみ |
| `SetCount` | `ZddManager.CountSets(zdd)` | ZDD のみ |

> **注意:** ライブラリの `DiagramStatistics` には `RedundantNodeCount`・`SharedNodeCount` は存在しない。設計書 v1.0 に記載されていたこれらのフィールドは削除した。ステータスバーの「冗長」表示には `ReachableNodeCount` と `TotalNodeCount` の差分を代替として使用する（IQ-06 で最終決定）。
...
classDiagram
    ...
    DiagramService "1" --> "1" DecisionDiagramManager : 保持（アプリ生涯）
    DiagramService ..> DiagramSession : 生成して返す
```

**ライフサイクル規則:**
- `DecisionDiagramManager` は `DiagramService` が DI で受け取り、アプリ生涯で1インスタンスのみ保持する
- ファミリー切り替えは Manager の再生成を行わない。`_currentBdd` 等の使用フィールドを切り替えるだけである（ADR-006）
- Manager は変数テーブルを蓄積する。新規ボタンで「セッションリセット」する場合は、新しい `DecisionDiagramManager` インスタンスを生成し直す（IQ-08）
...
- **変数名バリデーション** — `DiagramService.BuildAsync()` の入口で変数名を `[a-zA-Z_][a-zA-Z0-9_]*` パターンで検証し、不正な場合は `ArgumentException` をスローする（IQ-09 で文字セットを最終決定）
...
{
    // virtual host mapping によるローカルコンテンツ以外はキャンセル
    if (!e.Uri.StartsWith("https://diagrams.local/")) e.Cancel = true;
};
```

**CSP ヘッダー（virtual host mapping 経由で付与）:**
...
- 変数名の許容パターン: `^[a-zA-Z_][a-zA-Z0-9_]*$`（IQ-09 で最終決定）
...
- `DiagramService` が内部エラーで不整合状態になった場合は `DecisionDiagramManager` を再生成して新しいインスタンスを DI から再取得するリカバリーパスを持つ
...
**決定:**
1. SVG は `NavigateToString()` で読み込むが、`virtual host mapping` 経由の CSP ヘッダーで `script-src` を nonce 制限する
...
**代替案:** SVG を `<img src="data:image/svg+xml,...">` で表示（スクリプト実行なし）→ノードクリックが取れないため不採用
...
| FR-BDD-005 | 統計情報表示 | `AppDiagramStatistics` ← `BddManager.GetStatistics()` | `ReachableNodeCount` と `TotalNodeCount` の整合性 |
...
以下はアーキテクチャレベルで決定が必要な残課題である。

| ID | 分類 | 事項 | 優先度 | 影響フェーズ |
|---|---|---|---|---|
| IQ-06 | 統計 | ステータスバーの「冗長ノード数（Low==High）」はライブラリ API に存在しない。`GetReachableNodeViews()` を走査して Low==High を数えるか、表示仕様を `TotalNodeCount - ReachableNodeCount` に変更するかを決定する | 高 | v0.1 |
| IQ-07 | セキュリティ | SVG → WebView2 への表示方式の最終確認: `NavigateToString` + CSP vs virtual host mapping の実装難易度検証 | 高 | v0.1 |
| IQ-08 | ライフサイクル | 「新規」ボタン時に `DecisionDiagramManager` を再生成するか否か。再生成すると変数テーブルをリセットできるが、DI での再登録が必要になる。再生成しない場合は変数名の蓄積を許容する | 中 | v0.1 |
| IQ-09 | セキュリティ | 変数名の許容文字セット: `[a-zA-Z_][a-zA-Z0-9_]*` のみか、日本語（Unicode 識別子）も許可するか。DOT エスケープとの整合を確認する | 中 | v0.1 |
| IQ-10 | 性能 | BDT 深さ制限: 変数数 > 10 で無効化（ADR-004）だが、BDT ノード数 `2^11-1 = 2047` は 1000 超確認ダイアログの対象。BDT 専用の上限警告 UI が必要か確認する | 低 | v0.1 |
